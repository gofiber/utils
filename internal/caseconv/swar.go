...
func FirstUpperIndex(b []byte) int {
	...
	i := 0
	for ; i+8 <= n; i += 8 {
		if m := UpperMask(binary.LittleEndian.Uint64(b[i:])); m != 0 {
	...
	if n >= 8 {
		...
		if m := UpperMask(binary.LittleEndian.Uint64(b[n-8:])); m != 0 {
...
func FirstLowerIndex(b []byte) int {
	...
	i := 0
	for ; i+8 <= n; i += 8 {
		if m := LowerMask(binary.LittleEndian.Uint64(b[i:])); m != 0 {
			return i + firstLane(m)
		}
	}
	if i == n {
	...
	if n >= 8 {
		if m := LowerMask(binary.LittleEndian.Uint64(b[n-8:])); m != 0 {
			return n - 8 + firstLane(m)
		}
		return -1
	}
...
func ToLowerInPlace(b []byte) {
	...
	i := 0
	for ; i+8 <= n; i += 8 {
		binary.LittleEndian.PutUint64(b[i:], ToLowerWord(binary.LittleEndian.Uint64(b[i:])))
	}
...
func ToUpperInPlace(b []byte) {
	...
	i := 0
	for ; i+8 <= n; i += 8 {
		binary.LittleEndian.PutUint64(b[i:], ToUpperWord(binary.LittleEndian.Uint64(b[i:])))
	}
	for ; i < n; i++ {
...
// byte offset from. Bytes before from must already be present in dst.
// len(dst) must equal len(src).
func ToLowerCopy(dst, src []byte, from int) {
	n := len(src)
	i := from
	for ; i+8 <= n; i += 8 {
		binary.LittleEndian.PutUint64(dst[i:], ToLowerWord(binary.LittleEndian.Uint64(src[i:])))
	}
	if i == n {
		return
	}
	if n >= 8 {
		binary.LittleEndian.PutUint64(dst[n-8:], ToLowerWord(binary.LittleEndian.Uint64(src[n-8:])))
		return
	}
	for ; i < n; i++ {
...
func ToUpperCopy(dst, src []byte, from int) {
	...
	i := from
	for ; i+8 <= n; i += 8 {
		binary.LittleEndian.PutUint64(dst[i:], ToUpperWord(binary.LittleEndian.Uint64(src[i:])))
	}
	if i == n {
		return
	}
	if n >= 8 {
		binary.LittleEndian.PutUint64(dst[n-8:], ToUpperWord(binary.LittleEndian.Uint64(src[n-8:])))
		return
	}
	for ; i < n; i++ {
